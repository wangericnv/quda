...
#undef flip
  ...
  Dirac* Dirac::create(const DiracParam &param)
  {
    ...
    } else if (param.type == QUDA_DWF_PAULI_DAGGER_DIRAC) {
      if (getVerbosity() >= QUDA_DEBUG_VERBOSE) printfQuda("Creating a DwfPauliDagger operator\n");
      return new DiracDwfPauliDagger(param);
