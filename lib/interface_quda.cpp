...
void flushChronoQuda(int i)
{
  ...
  for (auto & basi : basis) {
    if (basi.first)  delete basi.first;
    if (basi.second) delete basi.second;
  }
  basis.clear();
}
...
cudaGaugeField *gauge, cudaGaugeField *fatGauge, cudaGaugeField *longGauge, cudaCloverField *clover)
  {
    ...
    diracParam.gauge = (gauge == nullptr ? gaugePrecise : gauge);
    diracParam.fatGauge = (fatGauge == nullptr ? gaugeFatPrecise : fatGauge);
    diracParam.longGauge = (longGauge == nullptr ? gaugeLongPrecise : longGauge);
    diracParam.clover = (clover == nullptr ? cloverPrecise : clover);
  ...
  {
    setDiracParam(diracParam, inv_param, pc, gaugeSloppy, gaugeFatSloppy, gaugeLongSloppy, cloverSloppy);
...
multigrid_solver::multigrid_solver(QudaMultigridParam &mg_param, TimeProfile &profile)
  ...
  ColorSpinorParam cpuParam(nullptr, *param, cudaGauge->X(), pc_solution, QUDA_CPU_FIELD_LOCATION);
...
void computeKSLinkQuda(void* fatlink, void* longlink, void* ulink, void* inlink, double *path_coeff, QudaGaugeParam *param) {
  ...
  cudaGaugeField *cudaInLinkEx = createExtendedGauge(*cudaInLink, R, profileFatLink, true);
...
void compute_gauge_force_quda_(void *mom, void *gauge, int *num_loop_types, double *coeff, double *dt,
  ...
  auto *path_length = static_cast<int*>(safe_malloc(numPaths*sizeof(int)));
...
void gaussGaugeQuda(long seed)
{
  ...
  auto* randstates = new RNG(data->Volume(), seed, data->X());
  randstates->Init();
